...
THREEx.dilateGeometry = function (geometry, length) {
  ...
  geometry.vertices.forEach(function (vertex, idx) {
    ...
    if (vertexNormal != null) {
      ...
      console.warn('THREEx DilateGeometry : Geometry problem')
    }
  })
}
...
THREEx.ToxicPproc.passesPreset['sober'] = {
  init: function () {
    // hBlurPass
    let uniforms = this.hBlurTween.dstUniforms
    uniforms.h.value = 0

    // vBlurPass
    uniforms = this.vBlurTween.dstUniforms
    uniforms.v.value = 0

    // rgbRadialPass
    uniforms = this.rgbRadialTween.dstUniforms
    uniforms.factor.value = 0
    uniforms.power.value = 3.0

    // seeDoublePass
    uniforms = this.seeDoubleTween.dstUniforms
    ...
    // refractionPass
    uniforms = this.refractionTween.dstUniforms
    uniforms.timeSpeed.value = 0
...
THREEx.ToxicPproc.Passes = function (presetLabel) {
  ...
  /**
   * to add toxicPasses to a EffectComposer
   * @param {EffectComposer} composer the composer to which it is added
   */
  ...
  onUpdateFcts.push(
    function (delta, now) {
      ...
      // seeDoublePass
      let uniforms = seeDoublePass.uniforms
      uniforms.time.value += delta * uniforms.timeSpeed.value
      // refractionPass
      uniforms = refractionPass.uniforms
      uniforms.time.value += delta * uniforms.timeSpeed.value
