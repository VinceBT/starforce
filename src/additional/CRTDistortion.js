...
// Adapted from https://www.shadertoy.com/view/llsfD8
...
const CRTDistortion = {
  uniforms: {
    ...
    bulge: { type: 'f', value: 0.1 },
    rgbAmount: { type: 'f', value: 0.05 },
    rgbPower: { type: 'f', value: 1.0 },
    vignetteScale: { type: 'f', value: 4.0 },
    vignettePower: { type: 'f', value: 2.0 },
  },
  vertexShader: [
    'varying vec2 vUv;',
  ...
  fragmentShader: `
    ...
    {
      float scrolling = step(0.0, sin(time * 6.28 / period) * 0.5 + (-0.5 + amount)); //set scrolling to 1 if frequency sin wave is above 0
    
      coord.y -= time * speed * scrolling; //if scrolling offset y coord over time
    ...
    void main() {
      ...
      sampleCoord = bulgeCoords(sampleCoord, sourceCoord, bulge); //apply bulge effect to sample coords
    
      vec4 outputColor = sampleRGBVignette(tDiffuse, sampleCoord, sourceCoord, rgbAmount, rgbPower); //sample bulged coords with color abberation
    
      float vignetteAmount = 1.2 + 0.05 * sin(time * 50.0); //set a fluctuating vignette intensity
      outputColor = applyVignette(outputColor, sourceCoord, vignetteAmount, vignetteScale, vignettePower); //apply vignette to sampled color
    
      // vec2 scanLineCoord = bulgeCoords(sourceCoord, sourceCoord, bulge * 0.2);  //get bulged coords to bulge scan lines
      // outputColor = applyScanLines(outputColor, scanLineCoord, 150.0, 0.25, 2.0, 0.01); //apply scan lines (with bulged coords)
    
      gl_FragColor = opacity * outputColor;
    }`,
}
